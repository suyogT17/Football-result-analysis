...
public class GoalScored {
	...
	public static class TokenizerMapper extends Mapper<Object, Text, Text, IntWritable> {
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			if(!(values[3]).equals("home_score")) {
				team1.set(values[1]);
				context.write(team1, new IntWritable(Integer.parseInt(values[3])));
			}

			if(!(values[4]).equals("away_score")) {
				team2.set(values[2]);
				context.write(team2, new IntWritable(Integer.parseInt(values[4])));
			}
		}
	...
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(GoalScored.class);

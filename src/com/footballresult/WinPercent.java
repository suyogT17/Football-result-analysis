...
public class WinPercent {
	...
	public static class TokenizerMapper extends Mapper<Object, Text, Text, Text> {
		...
		private Text loser = new Text();

		private Text w = new Text("W");
		private Text l = new Text("L");
		private Text t = new Text("T");
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			if(!(values[3]).equals("home_score") && !(values[4]).equals("away_score") ) {
				...
				if(team1goals > team2goals) {
					winner.set(values[1]);
					loser.set(values[2]);
					context.write(winner, w);
					context.write(loser, l);
					...
					context.write(new Text(values[1]), t);
					context.write(new Text(values[2]), t);
				}
	...
	public static class IntSumReducer extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			float percent = (float)((float)won/(float)count)*100;
			context.write(key, new Text(percent+"%"));
	...
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(WinPercent.class);
